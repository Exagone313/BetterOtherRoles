...
using static BetterOtherRoles.Modules.CustomOption;
...
namespace BetterOtherRoles.Modules {
    public class CustomOption {
        ...
        public string DisplayValue
        {
            get
            {
                ...
                {
                    return selection > 0 ? Helpers.cs(Color.green, "yes") : Helpers.cs(Color.red, "no");
                }

                return $"{selections[selection]}{suffix}";
            }
        }

        public string DisplayUIValue
        {
            get
            {
                if (valueType == ValueType.Boolean)
                {
                    return selection > 0 ? Helpers.cs(Color.green, "✔") : Helpers.cs(Color.red, "✖");
                }
        ...
        public CustomOption(int id, CustomOptionType type, string name,  System.Object[] selections, System.Object defaultValue, CustomOption parent, bool isHeader, string suffix, Action onChange = null, string heading = "") {
            this.id = id;
            ...
            selection = 0;
            if (id != 0) {
                entry = BetterOtherRolesPlugin.Instance.Config.Bind($"Preset{preset}", id.ToString(), defaultSelection);
                selection = Mathf.Clamp(entry.Value, 0, selections.Length - 1);
            }
            options.Add(this);
        }

        public static CustomOption Create(int id, CustomOptionType type, string name, string[] selections, CustomOption parent = null, bool isHeader = false, string suffix = "", Action onChange = null, string heading = "") {
            return new CustomOption(id, type, name, selections, "", parent, isHeader, suffix, onChange, heading);
        }

        public static CustomOption Create(int id, CustomOptionType type, string name, float defaultValue, float min, float max, float step, CustomOption parent = null, bool isHeader = false, string suffix = "", Action onChange = null, string heading = "") {
            List<object> selections = new();
            for (float s = min; s <= max; s += step)
                selections.Add(s);
            return new CustomOption(id, type, name, selections.ToArray(), defaultValue, parent, isHeader, suffix, onChange, heading);
        }

        public static CustomOption Create(int id, CustomOptionType type, string name, bool defaultValue, CustomOption parent = null, bool isHeader = false, string suffix = "", Action onChange = null, string heading = "") {
            return new CustomOption(id, type, name, new string[]{"Off", "On"}, defaultValue ? "On" : "Off", parent, isHeader, suffix, onChange, heading);
        }
        ...
        public static void switchPreset(int newPreset) {
            ...
            vanillaSettings = BetterOtherRolesPlugin.Instance.Config.Bind($"Preset{preset}", "GameOptions", "");
            loadVanillaOptions();
            foreach (CustomOption option in CustomOption.options) {
                if (option.id == 0) continue;

                option.entry = BetterOtherRolesPlugin.Instance.Config.Bind($"Preset{preset}", option.id.ToString(), option.defaultSelection);
        ...
        public static bool loadVanillaOptions() {
            ...
            if (gameOptions.Version < 8) {
                BetterOtherRolesPlugin.Logger.LogMessage("tried to paste old settings, not doing this!");
                return false;
            } 
        ...
        public static int deserializeOptions(byte[] inputValues) {
            ...
            while (reader.BaseStream.Position < inputValues.Length) {
                try {
                    ...
                    BetterOtherRolesPlugin.Logger.LogWarning($"id:{lastId}:{e}: while deserializing - tried to paste invalid settings!");
                    errors++;
                }
            }
        ...
        public static void copyToClipboard() {
            GUIUtility.systemCopyBuffer = $"{BetterOtherRolesPlugin.VersionString}!{Convert.ToBase64String(serializeOptions())}!{vanillaSettings.Value}";
        }
        ...
        public static int pasteFromClipboard() {
            ...
            try {
                ...
                ShareOptionSelections();
                if (BetterOtherRolesPlugin.Version > versionInfo && versionInfo < Version.Parse("4.6.0")) {
                ...
                BetterOtherRolesPlugin.Logger.LogWarning($"{e}: tried to paste invalid settings!\n{allSettings}");
    ...
    class LobbyViewSettingsPatch{
        ...
        public static void Postfix(LobbyViewSettingsPane __instance) {
            currentButtons.ForEach(UnityEngine.Object.Destroy);
        ...
        public static void removeVanillaTabs(LobbyViewSettingsPane __instance) {
            ...
            if(rolesTab)
                UnityEngine.Object.Destroy(rolesTab);
        ...
        public static void drawTab(LobbyViewSettingsPane __instance, CustomOption.CustomOptionType optionType) {
            ...
            var relevantOptions = options.Where(x => x.type == optionType || x.type == CustomOption.CustomOptionType.Guesser && optionType == CustomOption.CustomOptionType.General).ToList();
            ...
            for (int j = 0; j < __instance.settingsInfo.Count; j++) {
                UnityEngine.Object.Destroy(__instance.settingsInfo._items[j].gameObject);
            }
            ...
            int headers = 0;
            int lines = 0;
            var curType = CustomOption.CustomOptionType.Modifier;
            ...
            foreach (var option in relevantOptions) {
                if (option.isHeader && (int)optionType != 99 || (int)optionType == 99 && curType != option.type) {
                    ...
                    if ((int)optionType == 99)
                        categoryHeaderMasked.Title.text = new Dictionary<CustomOption.CustomOptionType, string>() { { CustomOption.CustomOptionType.Impostor, "Impostor Roles" }, { CustomOption.CustomOptionType.Neutral, "Neutral Roles" },
                            { CustomOption.CustomOptionType.Crewmate, "Crewmate Roles" }, { CustomOption.CustomOptionType.Modifier, "Modifiers" } }[curType];
                ...
                if (option.isHeader && (int)optionType != 99 && option.heading == "" && (option.type == CustomOption.CustomOptionType.Neutral || option.type == CustomOption.CustomOptionType.Crewmate || option.type == CustomOption.CustomOptionType.Impostor || option.type == CustomOption.CustomOptionType.Modifier)) {
                ...
                if ((int)optionType == 99) {
                    ...
                    if (option.type == CustomOption.CustomOptionType.Modifier)
        ...
        public static void createSettingTabs(LobbyViewSettingsPane __instance) {
            ...
            if (TORMapOptions.gameMode == CustomGamemodes.Guesser || TORMapOptions.gameMode == CustomGamemodes.Classic) {
                ...
                // create TOR settings
                createCustomButton(__instance, next++, "TORSettings", "TOR Settings", CustomOption.CustomOptionType.General);
                   // create TOR settings
                createCustomButton(__instance, next++, "RoleOverview", "Role Overview", (CustomOption.CustomOptionType)99);
                // IMp
                createCustomButton(__instance, next++, "ImpostorSettings", "Impostor Roles", CustomOption.CustomOptionType.Impostor);

                // Neutral
                createCustomButton(__instance, next++, "NeutralSettings", "Neutral Roles", CustomOption.CustomOptionType.Neutral);
                // Crew
                createCustomButton(__instance, next++, "CrewmateSettings", "Crewmate Roles", CustomOption.CustomOptionType.Crewmate);
                // Modifier
                createCustomButton(__instance, next++, "ModifierSettings", "Modifiers", CustomOption.CustomOptionType.Modifier);

            } else if (TORMapOptions.gameMode == CustomGamemodes.HideNSeek) {
                // create Main HNS settings
                createCustomButton(__instance, next++, "HideNSeekMain", "Hide 'N' Seek", CustomOption.CustomOptionType.HideNSeekMain);
                // create HNS Role settings
                createCustomButton(__instance, next++, "HideNSeekRoles", "Hide 'N' Seek Roles", CustomOption.CustomOptionType.HideNSeekRoles);
            } else if (TORMapOptions.gameMode == CustomGamemodes.PropHunt) {
                createCustomButton(__instance, next++, "PropHunt", "Prop Hunt", CustomOption.CustomOptionType.PropHunt);
            }
        }
    }
    ...
    class GameOptionsMenuStartPatch {
        ...
        public static void Postfix(GameSettingMenu __instance) {
            currentTabs.ForEach(UnityEngine.Object.Destroy);
            currentButtons.ForEach(UnityEngine.Object.Destroy);
            currentTabs = new();
            currentButtons = new();
            ...
            bgrenderer.sprite = Helpers.loadSpriteFromResources("BetterOtherRoles.Resources.CopyPasteBG.png", 175f);
            ...
            copyButtonRenderer.sprite = Helpers.loadSpriteFromResources("BetterOtherRoles.Resources.Copy.png", 100f);
            copyButton.transform.GetChild(1).transform.localPosition = Vector3.zero;
            copyButtonActiveRenderer.sprite = Helpers.loadSpriteFromResources("BetterOtherRoles.Resources.CopyActive.png", 100f);
            ...
            pasteButtonRenderer.sprite = Helpers.loadSpriteFromResources("BetterOtherRoles.Resources.Paste.png", 100f);
            pasteButtonActiveRenderer.sprite = Helpers.loadSpriteFromResources("BetterOtherRoles.Resources.PasteActive.png", 100f);
        ...
        private static void createSettings(GameOptionsMenu menu, List<CustomOption> options) {
            ...
            foreach (CustomOption option in options) {
                ...
                if (option.isHeader && option.heading == "" && (option.type == CustomOption.CustomOptionType.Neutral || option.type == CustomOption.CustomOptionType.Crewmate || option.type == CustomOption.CustomOptionType.Impostor || option.type == CustomOption.CustomOptionType.Modifier)) {
            ...
            for (int i = 0; i < menu.Children.Count; i++) {
                OptionBehaviour optionBehaviour = menu.Children._items[i];
        ...
        {
            UnityEngine.Object.Destroy(GameObject.Find("What Is This?"));
            UnityEngine.Object.Destroy(GameObject.Find("GamePresetButton"));
            UnityEngine.Object.Destroy(GameObject.Find("RoleSettingsButton"));
        ...
        public static void createGameOptionsMenu(GameSettingMenu __instance, CustomOption.CustomOptionType optionType, string settingName) {
        ...
        private static void createSettingTabs(GameSettingMenu __instance) {
            ...
            if (TORMapOptions.gameMode == CustomGamemodes.Guesser || TORMapOptions.gameMode == CustomGamemodes.Classic) {
                ...
                createGameOptionsMenu(__instance, CustomOption.CustomOptionType.General, "TORSettings");
                ...
                if (TORMapOptions.gameMode == CustomGamemodes.Guesser) {
                    ...
                    createGameOptionsMenu(__instance, CustomOption.CustomOptionType.Guesser, "GuesserSettings");
                }
                // IMp
                createCustomButton(__instance, next++, "ImpostorSettings", "Impostor Roles");
                createGameOptionsMenu(__instance, CustomOption.CustomOptionType.Impostor, "ImpostorSettings");

                // Neutral
                createCustomButton(__instance, next++, "NeutralSettings", "Neutral Roles");
                createGameOptionsMenu(__instance, CustomOption.CustomOptionType.Neutral, "NeutralSettings");
                // Crew
                createCustomButton(__instance, next++, "CrewmateSettings", "Crewmate Roles");
                createGameOptionsMenu(__instance, CustomOption.CustomOptionType.Crewmate, "CrewmateSettings");
                // Modifier
                createCustomButton(__instance, next++, "ModifierSettings", "Modifiers");
                createGameOptionsMenu(__instance, CustomOption.CustomOptionType.Modifier, "ModifierSettings");

            } else if (TORMapOptions.gameMode == CustomGamemodes.HideNSeek) {
                // create Main HNS settings
                createCustomButton(__instance, next++, "HideNSeekMain", "Hide 'N' Seek");
                createGameOptionsMenu(__instance, CustomOption.CustomOptionType.HideNSeekMain, "HideNSeekMain");
                // create HNS Role settings
                createCustomButton(__instance, next++, "HideNSeekRoles", "Hide 'N' Seek Roles");
                createGameOptionsMenu(__instance, CustomOption.CustomOptionType.HideNSeekRoles, "HideNSeekRoles");
            } else if (TORMapOptions.gameMode == CustomGamemodes.PropHunt) {
                createCustomButton(__instance, next++, "PropHunt", "Prop Hunt");
                createGameOptionsMenu(__instance, CustomOption.CustomOptionType.PropHunt, "PropHunt");
            }
        }
    }
    ...
    class GameOptionsDataPatch
    {
        ...
        private static string buildOptionsOfType(CustomOption.CustomOptionType type, bool headerOnly) {
            ...
            foreach (CustomOption option in options) {
                if (TORMapOptions.gameMode == CustomGamemodes.HideNSeek && option.type != CustomOption.CustomOptionType.HideNSeekMain && option.type != CustomOption.CustomOptionType.HideNSeekRoles) continue;
                if (TORMapOptions.gameMode == CustomGamemodes.PropHunt && option.type != CustomOption.CustomOptionType.PropHunt) continue;
                if (option.parent != null) {
        ...
        public static string buildAllOptions(string vanillaSettings = "", bool hideExtras = false) {
            ...
            int counter = BetterOtherRolesPlugin.optionsPage;
            ...
            if (TORMapOptions.gameMode == CustomGamemodes.HideNSeek) {
                if (BetterOtherRolesPlugin.optionsPage > 1) BetterOtherRolesPlugin.optionsPage = 0;
                maxPage = 2;
                switch (counter) {
                    case 0:
    ...
    public static class GameOptionsNextPagePatch
    {
        ...
        {
            int page = BetterOtherRolesPlugin.optionsPage;
            if (Input.GetKeyDown(KeyCode.Tab)) {
                BetterOtherRolesPlugin.optionsPage = (BetterOtherRolesPlugin.optionsPage + 1) % 7;
            }
            if (Input.GetKeyDown(KeyCode.Alpha1) || Input.GetKeyDown(KeyCode.Keypad1)) {
                BetterOtherRolesPlugin.optionsPage = 0;
            }
            if (Input.GetKeyDown(KeyCode.Alpha2) || Input.GetKeyDown(KeyCode.Keypad2)) {
                BetterOtherRolesPlugin.optionsPage = 1;
            }
            if (Input.GetKeyDown(KeyCode.Alpha3) || Input.GetKeyDown(KeyCode.Keypad3)) {
                BetterOtherRolesPlugin.optionsPage = 2;
            }
            if (Input.GetKeyDown(KeyCode.Alpha4) || Input.GetKeyDown(KeyCode.Keypad4)) {
                BetterOtherRolesPlugin.optionsPage = 3;
            }
            if (Input.GetKeyDown(KeyCode.Alpha5) || Input.GetKeyDown(KeyCode.Keypad5)) {
                BetterOtherRolesPlugin.optionsPage = 4;
            }
            if (Input.GetKeyDown(KeyCode.Alpha6) || Input.GetKeyDown(KeyCode.Keypad6)) {
                BetterOtherRolesPlugin.optionsPage = 5;
            }
            if (Input.GetKeyDown(KeyCode.Alpha7) || Input.GetKeyDown(KeyCode.Keypad7)) {
                BetterOtherRolesPlugin.optionsPage = 6;
            }
            if (Input.GetKeyDown(KeyCode.F1))
                HudManagerUpdate.ToggleSettings(HudManager.Instance);
            if (BetterOtherRolesPlugin.optionsPage >= GameOptionsDataPatch.maxPage) BetterOtherRolesPlugin.optionsPage = 0;
        }
    }
    ...
    public class HudManagerUpdate {
        ...
        public static void CloseSettings() {
            ...
            if (settingsBackground) UnityEngine.Object.Destroy(settingsBackground);
        }
        ...
        public static void Postfix(HudManager __instance) {
            ...
            if (!toggleSettingsButton || !toggleSettingsButtonObject) {
                ...
                renderer.sprite = Helpers.loadSpriteFromResources("BetterOtherRoles.Resources.Settings_Button.png", 100f);
                rendererActive.sprite = Helpers.loadSpriteFromResources("BetterOtherRoles.Resources.Settings_ButtonActive.png", 100);
            ...
            if (!toggleZoomButton || !toggleZoomButtonObject) {
                ...
                tZrenderer.sprite = Helpers.loadSpriteFromResources("BetterOtherRoles.Resources.Minus_Button.png", 100f);
                tZArenderer.sprite = Helpers.loadSpriteFromResources("BetterOtherRoles.Resources.Minus_ButtonActive.png", 100);
