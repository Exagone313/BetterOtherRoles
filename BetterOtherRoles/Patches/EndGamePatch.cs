...
namespace BetterOtherRoles.Patches {
    ...
    public class OnGameEndPatch {
        ...
        public static void Postfix(AmongUsClient __instance, [HarmonyArgument(0)]ref EndGameResult endGameResult) {
            ...
            if (UnknownImpostors.IsEnabled)
            {
                ...
                try
                {
                    var exiledPlayer = ExileController.Instance.initData?.networkedPlayer?.Object;
            ...
            if (Lawyer.lawyer != null && Lawyer.target != null && (!Lawyer.target.Data.IsDead || Lawyer.target == Jester.jester) && !Pursuer.notAckedExiled && !Lawyer.isProsecutor) {
                ...
                if (winningClient != null) { // The Lawyer wins if the client is winning (and alive, but if he wasn't the Lawyer shouldn't exist anymore)
                    if (EndGameResult.CachedWinners.ToArray().All(x => x.PlayerName != Lawyer.lawyer.Data.PlayerName))
            ...
            if (UnknownImpostors.IsBattleRoyale)
            {
                winners = PlayerControl.AllPlayerControls.ToArray()
                    ...
                    .Select(x => new CachedPlayerData(x.Data))
                    .ToList();
                
                EndGameResult.CachedWinners = new Il2CppSystem.Collections.Generic.List<CachedPlayerData>();
                EndGameResult.CachedWinners.Add(winners.First());
            }
    ...
    public class EndGameManagerSetUpPatch {
        public static void Postfix(EndGameManager __instance) {
            ...
            // PRANKEX WHERE :)
            List<CachedPlayerData> list = EndGameResult.CachedWinners.ToArray().Where(x => !UnknownImpostors.IsBattleRoyale || !x.IsDead).ToList().OrderBy(delegate(CachedPlayerData b)
            {
                if (!b.IsYou)
                {
                    return 0;
    ...
    class CheckEndCriteriaPatch {
        public static bool Prefix(ShipStatus __instance) {
            ...
            if (DevConfig.DisableEndGameConditions) return false;
